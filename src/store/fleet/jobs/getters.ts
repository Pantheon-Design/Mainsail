...
export const getters: GetterTree<FleetJobsState, any> = {
    ...
    getHighPriorityJobsCount(state): number {
        return state.jobs.filter((job: FleetJob) => job.priority >= 5).length
    },

    getMediumPriorityJobsCount(state): number {
        return state.jobs.filter((job: FleetJob) => job.priority >= 3 && job.priority < 5).length
    },

    getLowPriorityJobsCount(state): number {
        return state.jobs.filter((job: FleetJob) => job.priority < 3).length
    },
