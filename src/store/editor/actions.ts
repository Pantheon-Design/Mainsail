...
export const actions: ActionTree<EditorState, RootState> = {
    ...
    openFile({ state, dispatch, commit, rootGetters, rootState }, payload) {
        ...
        // UL 2011 Safety Compliant mode: force read-only regardless of file permissions.
        const safetyOn = rootState.gui?.general?.ul2011SafetyCompliant === true
        commit('setPermissions', safetyOn ? 'r' : payload.permissions)
    ...
    regeneratePrinterConfig({ state, commit, getters, rootGetters, dispatch }, payload: { content: string }) {
        const content = new Blob([payload.content], { type: 'text/plain' })
        ...
        axios
            ...
            .then((data) => {
                if (data == null || data === '') {
                    ...
                    axios
                        ...
                        .then((data) => {
                            dispatch('clearLoader')
                            Vue.$toast.success(
                                i18n.t('Editor.SuccessfullySaved', { filename: data.item.path }).toString()
                            )
                            commit('updateLoadedHash', payload.content)
                        })
                        .catch((error) => {
                ...
                Vue.$socket.emit('server.files.delete_file', { path: 'config' + '/' + 'temp_features.yml' }, {})
            })
            .catch((error) => {
                window.console.log(error.response?.data.error)
                dispatch('clearLoader')
