...
**Fleet status page:** `src/pages/FleetStatus.vue` (`/fleet-status`) renders `src/components/panels/FleetUptimeTimeline.vue` — a status.claude.com-style 90-day uptime bar for the local fleet_daemon (plus rows for its cloud sync and its NAS connection when enabled), derived client-side from the process-life segments `GET <daemon>/daemon/uptime` returns; refreshes every minute while visible, and an unreachable daemon flips the row to offline while keeping the last timeline. The cloud dashboard repo carries the same component at `/status` (one row per site).
